"""Minimal end-to-end example using the Bernoulli-EKF tracker.

Loads the cached `apple_in_the_tray` trajectory's RGB + depth +
detections + SLAM/EE poses, drives the orchestrator-equivalent
``InstrumentedTracker`` for a few frames, and prints the per-frame
...
def main():
    ...
    tracker = viz.InstrumentedTracker(K=viz.K_DEFAULT, bernoulli_cfg=cfg)
    ...
    for idx in range(488, 525):
        ...
        gs = grip.step(width=width,
                       tracker_state=viz.InstrumentedTrackerState(tracker),
                       T_wb=T_wb, T_bg=T_bg,
