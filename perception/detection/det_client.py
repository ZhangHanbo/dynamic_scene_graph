...
def infer_image(
    ...
    """Send one RGB frame to the OWL+SAM detection server.

    Args:
        rgb:        (H, W, 3) uint8 RGB image.
        server_url: base URL of the detection server.
        vocabulary: per-request list of object queries; falls back to
                    the server-side OBJECTS list when None.
        history:    SAM2 video-tracker memory carried frame-to-frame.
                    The current /infer endpoint is single-image (plain
                    SAM); ``history`` is accepted but unused. The
                    returned history is the same value the caller passed
                    in, so multi-frame loops can thread it through
                    without branching.
        timeout:    request timeout in seconds.

    Returns:
        ``(response_dict, history)``. ``response_dict["detections"]`` is
        a list of ``{detection: [{label, score}, ...], box: [...],
        mask: <base64-PNG>}`` entries.
    """
    ...
    files = {"file": ("frame.png", buf, "application/octet-stream")}
    data: Dict[str, Any] = {}
    if vocabulary is not None:
        data["vocabulary"] = json.dumps(list(vocabulary))
