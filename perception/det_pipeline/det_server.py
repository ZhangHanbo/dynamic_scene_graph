...
# ---- Per-request vocabulary cache --------------------------------------
# OWL is tokenized once with `OBJECTS` at startup. When a caller asks for
# a different vocabulary, we re-tokenize on the fly and cache the result
# so repeated calls with the same vocab don't pay the cost twice.
_VOCAB_CACHE: Dict[Tuple[str, ...], np.ndarray] = {}
_VOCAB_CACHE_MAX = 8


def _get_tokenized(vocabulary: List[str]) -> np.ndarray:
    """Return a (100, max_len) tokenized-queries array for `vocabulary`."""
    key = tuple(vocabulary)
    cached = _VOCAB_CACHE.get(key)
    if cached is not None:
        return cached
    if _mdl is None or _cfg is None:
        raise RuntimeError("OWL model not initialized")
    tok = np.array([
        _mdl.tokenize(q, _cfg.dataset_configs.max_query_length)
        for q in vocabulary
    ])
    tok = np.pad(tok, pad_width=((0, 100 - len(vocabulary)), (0, 0)),
                 constant_values=0)
    if len(_VOCAB_CACHE) >= _VOCAB_CACHE_MAX:
        _VOCAB_CACHE.pop(next(iter(_VOCAB_CACHE)))
    _VOCAB_CACHE[key] = tok
    return tok
...
async def infer(file: UploadFile = File(...),
    ...
    # If the caller supplied a vocabulary (JSON-encoded list of strings),
    # tokenize per-request and use it; otherwise fall back to the
    # server-startup OBJECTS list.
    active_objects: List[str] = OBJECTS
    active_tokenized = _tokenized
    if vocabulary:
        try:
            req_vocab = json.loads(vocabulary)
            if (isinstance(req_vocab, list)
                    and all(isinstance(s, str) for s in req_vocab)
                    and 0 < len(req_vocab) <= 100):
                active_objects = req_vocab
                with MODEL_LOCK:
                    active_tokenized = _get_tokenized(req_vocab)
    ...
    with MODEL_LOCK:
        results = process_array_to_detections(
            img, _predictor, _mdl, _variables, _jitted,
            active_tokenized, _owl_input_size,
