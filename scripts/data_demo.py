...
# Bootstrap repo root so top-level packages (heuristic_tracker, ekf_tracker,
# perception, utils, etc.) resolve regardless of cwd.
...
from heuristic_tracker.object_pose_updater import update_obj_pose_ee, update_obj_pose_icp, update_child_objects_pose_icp, icp_reappear, clear_child_fixed_observations
...
from perception.camera_pose_refiner import refine_camera_pose, should_skip_object_fusion
from heuristic_tracker.object_relation_graph import get_relation_graph
from heuristic_tracker.gravity_simulator import gravity_simulation
from perception.detection import hungarian_detection
from perception.detection.mask_extractor import extract_mask
import time
import json
...
def run_ekf_dataset(DATASET_PATH, config, args):
    """EKF tracker dispatch.

    The canonical EKF runner over a recorded dataset is
    ``scripts/visualize_ekf_tracking.py``, which assembles the full
    detection→PoseEstimator→orchestrator→render pipeline (which now
    drives the lifted ``GaussianEkfTracker`` from
    ``ekf_tracker.gaussian_ekf_tracker``). It expects a *trajectory name*
    that resolves under ``tests/visualization_pipeline/<name>/``, with
    cached perception under ``perception/`` and friends.

    To keep ``data_demo.py`` minimal and avoid a second, half-complete
    EKF pipeline, this function dispatches to ``visualize_ekf_tracking.py``
    via subprocess when the dataset path is one of the known trajectory
    names; otherwise it prints the canonical command and exits.
    """
    import subprocess
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    viz_script = os.path.join(repo_root, "scripts", "visualize_ekf_tracking.py")
    traj_name = os.path.basename(os.path.normpath(DATASET_PATH))
    cached_traj_dir = os.path.join(repo_root, "tests", "visualization_pipeline", traj_name)
    if os.path.isdir(cached_traj_dir):
    ...
    print(f"[EKF/{args.ekf_backend}] dataset '{traj_name}' has no cached "
          f"trajectory at {cached_traj_dir}.")
    print(f"  EKF over arbitrary dataset paths is not yet wired up here.")
    print(f"  For a known trajectory, run:")
    print(f"    python3 scripts/visualize_ekf_tracking.py --trajectory <name>")
    print(f"  For the full EKF API, see ekf_tracker.TwoTierOrchestratorGaussian "
          f"and examples/ekf_offline.py.")
...
def parse_args():
    ...
    parser.add_argument('--tracker', choices=['heuristic', 'ekf'], default='heuristic',
                       help="Tracker variant: 'heuristic' (TSDF + ID) or 'ekf' (probabilistic).")
...
if __name__ == "__main__":
    ...
    if args.tracker == "ekf":
        run_ekf_dataset(DATASET_PATH, config, args)
    else:
