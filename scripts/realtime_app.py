...
--------------------------------
- Merges the offline pipeline in `data_demo.py` with the ROS capture logic in `scripts.rosbag2dataset.py`.
...
from perception.detection.mask_extractor import extract_mask  # You can still use this if you want
from perception.detection.det_client import detect_image
from perception.detection import hungarian_detection
from heuristic_tracker.id_associator import associate_by_id
from heuristic_tracker.object_pose_updater import (
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--tracker",
        choices=["heuristic", "ekf"],
        default="heuristic",
        help="Tracker variant. EKF mode is not yet wired to live ROS "
             "topics — the EKF backend needs SLAM covariances which the "
             "current pose subscriber doesn't publish. Use scripts/"
             "visualize_ekf_tracking.py for offline EKF on recorded data.",
    )
    args, unknown = parser.parse_known_args()  # 保留其他 ROS 参数

    if args.tracker == "ekf":
        raise NotImplementedError(
            "Real-time EKF tracker is not yet wired up. The EKF backend "
            "(ekf_tracker.TwoTierOrchestratorGaussian) requires per-frame "
            "SLAM covariances which the current ROS pose topic does not "
            "publish. To run the EKF tracker on a recorded trajectory, "
            "use:\n  python3 scripts/visualize_ekf_tracking.py --trajectory <name>"
        )
